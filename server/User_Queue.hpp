...
public:
    User(tcp::socket sock)
        : _con(std::move(sock)),
          _nodes_queue(sock.get_executor()),
          _config(nullptr),
    ...
    {
        {
            ...
            while (!_nodes_queue.empty())
                nodes_tmp.push_back(_nodes_queue.pop());
        ...
        for (;;)
        {
            ...
            std::cerr << _dbg_pre << "Received invalid message" << std::endl;
            co_return false;
        }
    }
    ...
    awaitable<void> _dispatch_input(Message::message_s* input)
    {
        ...
        auto node = co_await _nodes_queue.async_pop();
...
private:
    ...
    Queue<std::shared_ptr<Connection>> _nodes_queue;
